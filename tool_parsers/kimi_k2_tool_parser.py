...
Ported from vllm/tool_parsers/kimi_k2_tool_parser.py, adapted to use
project-local base classes and types (no vLLM dependency).
...
class KimiK2ToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: Any, tools: list[dict] | None = None):
        ...
        self.max_section_chars: int = 8192
        ...
        self.stream_tool_call_portion_regex = re.compile(
            r"(?P<tool_call_id>.+:\d+)\s*<\|tool_call_argument_begin\|>\s*"
    ...
    def extract_tool_calls_streaming(
        ...
        # Early return: if no section token detected yet, return as content
        ...
        try:
            ...
            # case: generating text or rounding out a tool call
            if (
            ...
            # case -- closing current tool call
            elif (
                cur_tool_start_count == cur_tool_end_count
            ...
            # case -- otherwise just generating text
            else:
                if self.in_tool_section:
            ...
            # Send tool name if not yet sent
            ...
            # prev arguments defined but none now (should be impossible)
